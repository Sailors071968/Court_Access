...
if [ -n "${PID:-}" ] && [ -r "/proc/$PID/environ" ]; then
  ...
  for V in NODE_ENV PORT HOST DATABASE_URL REDIS_URL REDIS_HOST JWT_SECRET JWT_REFRESH_SECRET COOKIE_SECRET FRONTEND_URL \
           CERTIFICATION_STAGING_DIR EVIDENCE_STORAGE_DIR AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_REGION S3_BUCKET \
    ...
    case "$V" in
      NODE_ENV|PORT|HOST|AWS_REGION|SES_REGION|FRONTEND_URL|CERTIFICATION_STAGING_DIR|EVIDENCE_STORAGE_DIR)
        val "$V" "${LINE#*=}" ;;
      *) present "$V" "${LINE#*=}" ;;
    esac
  done
