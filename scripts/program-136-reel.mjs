...
// visits /dashboard/demo-defense (ILLUSTRATIVE DEMONSTRATION). No console-error
...
const OUT = process.env.OUT || 'reports/screenshots/program-136-reel';
mkdirSync(OUT, { recursive: true });
const CREDS = {
  email: process.env.REVIEW_EMAIL || 'rev116@staging.courtaccess.test',
...
async function login(page) {
  ...
  await page.addInitScript((t) => localStorage.setItem('court-access-token', t), token);
  return token;
}
...
async function run() {
  ...
  try {
    ...
    await login(rp);
    await rp.goto(`${BASE}/dashboard/demo-defense`, { waitUntil: 'networkidle' });
    ...
    await login(wp);
    await wp.goto(`${BASE}/dashboard/demo-defense`, { waitUntil: 'networkidle' });
