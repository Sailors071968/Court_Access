  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/5 p-4">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-100">Case Law Search</h3>
      ...
      <form onSubmit={run} className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-md border border-white/10 px-3 py-2 text-sm focus:border-slate-500 focus:outline-none"
        />
      ...
      {searched && !loading && !error && count !== null && (
        <p className="mt-3 text-xs text-slate-400">{count.toLocaleString()} matching opinions · showing {results.length}</p>
      )}
      {searched && !loading && !error && results.length === 0 && (
        <p className="mt-3 text-sm text-slate-400">No opinions matched.</p>
      )}
        ...
        {results.map((r) => (
          <li key={r.opinionId ?? r.url ?? r.caseName} className="rounded-md border border-slate-100 p-3">
            <div className="flex items-start justify-between gap-2">
              <a href={r.url ?? '#'} target="_blank" rel="noreferrer" className="text-sm font-medium text-slate-100 hover:underline">
                {r.caseName}
              </a>
            ...
            {r.snippet && <p className="mt-1 line-clamp-3 text-xs text-slate-300">{r.snippet}</p>}
          </li>
        ))}
      </ul>
    </div>
