...
export const LOCKED_TABLES_REGISTRY: LockedTableDefinition[] = [
  ...
  // ----- PolicyDocument -----
  {
    ...
    columns: [
      ...
      { name: 'sha256Hash', type: 'String', nullable: true, description: 'SHA-256 hash for duplicate detection' },
      { name: 'topicConfidenceScore', type: 'Float', nullable: true, description: 'Phase 70 — topic classification confidence' },
      { name: 'needsManualReview', type: 'Boolean', nullable: false, defaultValue: 'false', description: 'Phase 70 — flagged if confidence < 0.65' },
      { name: 'createdAt', type: 'DateTime', nullable: false, defaultValue: 'now()' },
      { name: 'updatedAt', type: 'DateTime', nullable: false },
    ],
    consumers: [
