...
const OUT='/workspace/reports/screenshots/p93';
...
async function open(name,cid){
  const r=await pg.goto(`${U}/cases/${cid}/sentencing`,{waitUntil:'networkidle',timeout:40000});
  ...
  R.push(`${name}: HTTP ${r?.status()} | sections=${sections.length} [${sections.slice(0,5).join(', ')}...] | UNKNOWN=${unknowns}`);
...
try{
  ...
  const [dl]=await Promise.all([pg.waitForEvent('download',{timeout:15000}), pg.$$eval('button', els=>{const el=[...els].find(e=>/Sentencing Notebook/.test(e.textContent));if(el)el.click();})]);
  const fn=dl.suggestedFilename(); await dl.saveAs(`${OUT}/${fn}`);
  R.push('DOCX Sentencing Notebook: '+fn);
